...
from torchvision.models import alexnet, resnet50, densenet121, inception_v3, vgg16_bn, resnet18, resnet34, mobilenet_v3_small, mobilenet_v2, efficientnet_v2_m, swin_v2_b
...
if __name__ == "__main__":
    ...
    for model_no, model in model_dict.items():
        if model_no == "swin_v2_b":
            model = model(weights=None, num_classes=2)
        else:
            # Load Model
            model = model(pretrained=False, num_classes=2)

        stats_text = ""
        for seed in [0, 1408, 3074]:
            path = 'results/BCNB'
        ...
        with open(f"model_{model_no}.txt", "w") as f:
            f.write(stats_text)