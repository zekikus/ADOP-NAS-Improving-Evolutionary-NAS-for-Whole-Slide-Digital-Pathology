...
class Model(nn.Module):
    ...
    def __init__(self, chromosome = None, config = None, nbr_cell = None, nbr_stack = None, nbr_filters = None, num_classes = None, C_in = 3):
        super(Model, self).__init__()
        # CONSTANT
        self.NUM_VERTICES = 6
        self.MAX_EDGE_NBR = int((self.NUM_VERTICES * (self.NUM_VERTICES - 3)) / 2)
    ...
    def create_matrix(self):
        ...
        mtx_idx = 0
        for idx, node in enumerate(self.config[:self.MAX_EDGE_NBR]):
            if node != 0: 
                self.matrix[mtx_idx][node - 1] = 1
    ...
    def evaluate(self, train_loader, val_loader, loss_fn, metric_fn, device):
        ...
        try:
            ...
            for epoch in range(30):
                ...
                # Train Phase
                self.train()
                for inputs, _, _, labels in tqdm.tqdm(train_loader):
                ...
                self.eval()
                with torch.no_grad():
                    for inputs, _, _, labels in tqdm.tqdm(val_loader):
                        labels = labels.type(torch.LongTensor)
                        inputs, labels = inputs.to(device), labels.to(device)
                        output = self.forward(inputs)
