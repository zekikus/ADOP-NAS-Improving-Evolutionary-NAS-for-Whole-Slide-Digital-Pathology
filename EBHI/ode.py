...
"""
    ...
    TODO:
    - Bağlantılar için sıfır seçilme olasılığını kaldır. Zaten aynı düğümlerin tekrar seçilme olasılığı var
    - Her bir düğüm için bağlantı sayısını 3'e çıkar
"""
...
if __name__ == "__main__":
    ...
    seed = 42
    random.seed(seed)
    data_flag = f"EBHI_{seed}"
    NUM_CLASSES = 2
    BATCH_SIZE = 64

    os.makedirs(f"results/{data_flag}/", exist_ok=True)
    ...
    train_dataset = EBHI_Dataset(split='train', nas_stage=True, percentage=0.04)
    train_loader = DataLoader(train_dataset, batch_size=BATCH_SIZE, shuffle=True)
    
    val_dataset = EBHI_Dataset(split='val', nas_stage=True, percentage=0.02)
